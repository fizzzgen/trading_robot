...
def test_sell__order_move():
    ...
    def patched_configure_poloniex():
        ...
        polo.returnTradeHistory.return_value = []
        polo.moveOrder.return_value = {'type': 'sell', 'orderNumber': order_number2, 'rate': 1, 'amount': 1}
        return polo
    ...
    with patch1, patch2:
        ...
        with db.session_scope() as session:
            ...
            assert len(transaction) == 1
            assert transaction[0].id == order_number2
            assert transaction[0].price == buy_price / config.ORDERBOOK_FORCER_MOVE_PERCENT
