...
def process_sell(pair):
    ...
    with db.session_scope() as session:
        ...
        trades = polo.fetch_trades(symbol=pair)
        old_sell_trades_ids = [i[0] for i in session.query(db.Trade.id).all()]

        new_trades = list(map(attrdict.AttrDict,filter(lambda tr: (tr['globalTradeID'] not in old_sell_trades_ids and tr['type'] == 'buy'), trades)))

        balance = attrdict.AttrDict(polo.fetch_balance()[config.get_pair_second_symbol(pair)]).available
